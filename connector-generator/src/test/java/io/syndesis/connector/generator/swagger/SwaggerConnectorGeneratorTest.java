...
public class SwaggerConnectorGeneratorTest {
    ...
    public void shouldSculptForConcurQuickExpensesSwagger() throws IOException {
        ...
        for (final Action expectedAction : expected.getActions()) {
            final String actionId = expectedAction.getId().get();
            final Optional<Action> maybeSculptedAction = sculpted.actionById(actionId);
            assertThat(maybeSculptedAction).as("No action with id: " + actionId + " was sculpted").isPresent();

            final Action sculptedAction = maybeSculptedAction.get();
            assertThat(sculptedAction).as("Difference found for action: " + actionId)
                .isEqualToIgnoringGivenFields(expectedAction, "definition");

            assertThat(sculptedAction.getDefinition().getPropertyDefinitionSteps())
                .as("Sculpted and expected action definition property definition steps for action with id: " + actionId
                    + " differ")
                .isEqualTo(expectedAction.getDefinition().getPropertyDefinitionSteps());

            if (expectedAction.getDefinition().getInputDataShape().isPresent()) {
                final DataShape sculptedInputDataShape = sculptedAction.getDefinition().getInputDataShape().get();
                final DataShape expectedInputDataShape = expectedAction.getDefinition().getInputDataShape().get();
            ...
            if (expectedAction.getDefinition().getOutputDataShape().isPresent()) {
                final DataShape sculptedOutputDataShape = sculptedAction.getDefinition().getOutputDataShape().get();
                final DataShape expectedOutputDataShape = expectedAction.getDefinition().getOutputDataShape().get();
