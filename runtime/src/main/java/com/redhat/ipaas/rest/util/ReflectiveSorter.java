...
public class ReflectiveSorter<T> implements Function<List<T>, List<T>>, Comparator<T> {
  ...
  private Comparator<T> createDelegateComparator(Class<T> modelClass, String fieldName) {
    ...
    throw new IllegalArgumentException(
        String.format(
            "Cannot find field %s in %s as int or String field", fieldName, modelClass.getName()));
  }
      ...
      return Comparator.comparing(
          k -> {
            try {
              ...
              throw new IllegalArgumentException(
                  "Cannot extract String value from " + stringGetMethod + " for object " + k, e);
            }
          });
    }
    return null;
      ...
      return Comparator.comparingInt(
          k -> {
            try {
              ...
              throw new IllegalArgumentException(
                  "Cannot extract int value from " + intGetMethod + " for object " + k, e);
            }
          });
    }
    return null;
